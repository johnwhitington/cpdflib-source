...
/* cpdf_impose(x, y, fit, columns, rtl, btt, center, margin, spacing,
 * linewidth, pdf) imposes a PDF. There are two modes: imposing x * y, or
 * imposing to fit a page of size x * y. This is controlled by fit. Columns
 * imposes by columns rather than rows. rtl is right-to-left, btt
 * bottom-to-top. Center is unused for now. Margin is the margin around the
 * output, spacing the spacing between imposed inputs. */
void cpdf_impose(double, double, int, int, int, int, int, double, double, double, int);
