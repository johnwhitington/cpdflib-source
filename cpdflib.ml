...
=
  ...
  and color = color.r, color.g, color.b in
    let newpdf =
      (Cpdf.addtexts
         metrics
         1.0 outline false fontname (Some font) bates color position linespacing fontsize
         underneath text (Array.to_list range) Cpdf.Horizontal cropbox opacity justification midline filename pdf)
    in
      if not metrics then
        begin
...
let squeeze file_in file_out =
  Cpdfcommand.go_withargv
   [|"cpdf"; file_in; "-squeeze"; "-recrypt"; "-o"; file_out|];
  27.4
